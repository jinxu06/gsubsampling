...
from modules.eqv_vae import EquivariantVAE
...
# Component VAE
flags.DEFINE_boolean('comp_eqv_vae', False, 'Use EqvVAE as component?.')
...
class Genesis(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.comp_eqv_vae = cfg.comp_eqv_vae
        ...
        if self.two_stage:
            ...
            if self.comp_eqv_vae:
                if self.pixel_bound:
                    readout_fn = F.sigmoid
                self.comp_vae = EquivariantVAE(in_channels=3+1, 
                                            out_channels=3,
                                            img_size=cfg.img_size, 
                                            dim_latent=cfg.comp_ldim, 
                                            dim_group=2,
                                            activation=F.elu,
    ...
    def x_loss(x, log_m_k, x_r_k, std, pixel_wise=False):
        ...
        err_ppc = -torch.logsumexp(log_mx, dim=4) # -torch.log(log_mx.exp().sum(dim=4))
    ...
    def sample(self, batch_size, K_steps=None):
        ...
        if self.two_stage:
            ...
            if self.comp_prior:
                zc_k = []
                ps_k = []
                for step, zm in enumerate(zm_k):
                    mlp_out = self.prior_mlp(zm)
                    if self.comp_eqv_vae:
                        ps = self.comp_vae.z_eqv_to_ps(self.comp_vae.f2v(log_m_k[step].exp(), return_quotient=False))
                        ps_k.append(ps)
            ...
            if self.comp_eqv_vae:
                ps_batch = torch.cat(ps_k, dim=0)
                x_batch = self.comp_vae.decode(zc_batch, ps_batch)
            else:
