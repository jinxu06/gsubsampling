...
from genesis.utils.misc import loader_throughput, len_tfrecords, np_img_centre_crop

import multi_object_datasets.multi_dsprites as multi_dsprites
import multi_object_datasets.objects_room as objects_room
import multi_object_datasets.clevr_with_masks as clevr_with_masks
import multi_object_datasets.tetrominoes as tetrominoes


flags.DEFINE_string('data_folder', '',
                    'Path to data folder.')
flags.DEFINE_string('dataset', 'multi_dsprites',
...
TETROMINOS = '/tetrominoes_train.tfrecords'
OBJECTS_ROOM = '/objects_room/objects_room_train.tfrecords'
CLEVR = '/clevr_with_masks_train.tfrecords'
    ...
    def __next__(self):
        try:
            ...
            if self.img_size != shape[2]:
                img = F.interpolate(img, size=self.img_size, recompute_scale_factor=True)
            ...
            masks = torch.FloatTensor(masks)
            if self.img_size != shape[2]:
                masks = F.interpolate(masks, size=self.img_size, recompute_scale_factor=True)
...
config = forge.config()
tng_loader, val_loader, tst_loader = load(config)
images = []
segs = []
for idx, batch in enumerate(tng_loader):
  print('train', idx)
  images.append(batch['input'].numpy())
  segs.append(batch['instances'].numpy())
